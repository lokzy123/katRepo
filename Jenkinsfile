    ...
    stages {
		...
		stage('Execution') {
			steps{
				script {
					if(isBuildExecuted) {
						executeKatalon executeArgs: '-retry=0 -testSuitePath="${params.testSuitePath}" -browserType="${params.browser}" -executionProfile="${params.exeProfile}" -apiKey="b844dd8a-1ca5-4002-9b63-a7e7cd7f9b0e" --config -proxy.auth.option=NO_PROXY -proxy.system.option=NO_PROXY -proxy.system.applyToDesiredCapabilities=true -webui.autoUpdateDrivers=true', location: '', version: '10.0.1', x11Display: '', xvfbConfiguration: ''
					}
				}
			}
		}
